...
bool Raycast(const Vector2f& _origin, const Vector2f& _direction, const float _maxDistance,
	HitInfo& _hitInfo, const set<Actor*>& _ignoredActors, const float _precision)
{
	...
	{
		for (Actor* _actor : M_ACTOR.GetAllActors())
		{
			if (MeshActor* _meshActor = Cast<MeshActor>(_actor))
			{
				if (_ignoredActors.find(_meshActor) != _ignoredActors.end()) continue;
